    ...
    $('#sidebar-nav,#nav-col-submenu').on('click', '.dropdown-toggle', function (e) {
        ...
        if ($item.hasClass('open')) {
            $item.children('.submenu').slideDown('fast', function () {
                ...
                //$item.find('ul.submenu').css({overflow: "auto",height: _height3})
            });
        }
        else {
...
function GetLoadNav() {
    ...
    var _html = "";
    var level = 1;//定义导航栏的级别，暂时还没用到

    $.each(data, function (i) {
        var row = data[i];
        if (row.resourceowner == "0") {
            _html += GetLoadNavMenu(row, level); 
        }
    });
    $("#sidebar-nav ul").prepend(_html);
};

function GetLoadNavMenu(row, level) {
    level += 1;
    _html = "";
    _html += '<li>';
    _html += '<a data-id="Athena' + row.id + '" href="#" class="dropdown-toggle"><i class="' + row.resourceleftico + '"></i><span>' + row.resourcename + '</span><i class="fa fa-angle-right drop-icon"></i></a>';
    ...
    if (childNodes.length > 0) {
        ...
        $.each(childNodes, function (i) {
            ...
            if (subrow.ChildNodes.length > 0)
                _html += GetLoadNavMenu(subrow,level);//如果存在下级导航栏，则递归调用GetLoadNavMenu（）函数，产生下级导航
            else {
                _html += '<li>';
                _html += '<a class="menuItem" data-id="Athena' + subrow.id + '" href="' + subrow.resourceurl + '" data-index="' + subrow.id + '">' + subrow.resourcename + '</a>';
                _html += '</li>';
            }
        });
        _html += '</ul>';
